...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const [d] = await db
    .select({
      ...
      available: sql<number>`count(*) filter (where ${donorProfiles.availabilityStatus} = 'AVAILABLE' and ${donorProfiles.verificationStatus} = 'VERIFIED')::int`,
...
export async function availabilityBreakdown(): Promise<Bucket[]> {
  const rows = await db
    .select({ key: donorProfiles.availabilityStatus, value: count() })
    .from(donorProfiles)
    .groupBy(donorProfiles.availabilityStatus);
  return rows.map((r) => ({ label: r.key.replaceAll("_", " "), value: r.value }));
}
