...
export async function POST(request: Request) {
  try {
    ...
    const existing = await db
      ...
      .where(eq(eligibilityRules.version, version))
      .limit(1);
    if (existing.length > 0) {
      throw validationError("This rule version already exists");
    }

    const minAge = num(body.minAge, "Minimum age", 16, 70);
    const maxAge = num(body.maxAge, "Maximum age", 17, 100);
    ...
    const [created] = await db
      ...
      .values({
        ...
        minAge,
        maxAge,
        minWeightKg: num(body.minWeightKg, "Minimum weight", 40, 120),
        defaultIntervalDays: num(body.defaultIntervalDays, "Default interval", 30, 365),
        maleIntervalDays: num(body.maleIntervalDays, "Male interval", 30, 365),
        femaleIntervalDays: num(body.femaleIntervalDays, "Female interval", 30, 365),
    ...
    await writeAudit({
      ...
      newState: { version, effectiveFrom, isActive: false },
    });
