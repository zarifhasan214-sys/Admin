...
export default async function DonorResponsesPage({
  ...
  if (status) filters.push(eq(donorRequests.status, status as "ACCEPTED"));
                ...
                {rows.map((row) => (
                  <tr key={row.response.id} className="hover:bg-slate-50/70">
                    ...
                    <Td className="whitespace-nowrap text-slate-600">
                      {fmtDate(row.response.donorAcceptedAt, lang, true)}
                    </Td>
                    <Td>
                      <StatusBadge
                        status={row.response.contactPermissionGranted ? "ACTIVE" : "NOT_AVAILABLE"}
                      />
                    </Td>
                    <Td className="whitespace-nowrap text-slate-600">
