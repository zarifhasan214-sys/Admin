...
export default async function DonorsPage({
  ...
  if (q) {
    ...
    const term = or(
      ...
      ilike(donorProfiles.phone, like),
  ...
  if (availability)
    filters.push(eq(donorProfiles.availability, availability as "AVAILABLE"));
  if (verification)
    filters.push(
  ...
  if (searchable)
    filters.push(eq(donorProfiles.isSearchable, searchable === "yes"));
  const where = filters.length ? and(...filters) : undefined;

  const rule = await getActiveRule();
  const interval = rule?.defaultIntervalDays ?? 120;
  ...
  const [rows, [totalRow], unions] = await Promise.all([
    db
      .select({
        ...
        isSearchable: donorProfiles.isSearchable,
      })
      .from(donorProfiles)
