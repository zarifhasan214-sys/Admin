...
export default async function EmergencyContactsPage({
  ...
  if (active) filters.push(eq(emergencyContacts.active, active === "yes"));

  const rows = await db
    .select()
    .from(emergencyContacts)
    .where(filters.length ? and(...filters) : undefined)
    .orderBy(desc(emergencyContacts.active), emergencyContacts.category);
                  ...
                  <Td>
                    {canManage ? (
                      <ActionButton
                        ...
                        payload={{ isActive: !c.active }}
                        tone={c.active ? "neutral" : "success"}
                        label={
                          c.active
                            ? lang === "bn"
                              ? "নিষ্ক্রিয়"
