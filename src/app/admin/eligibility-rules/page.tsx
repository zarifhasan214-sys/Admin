...
export default async function EligibilityRulesPage() {
  ...
  const rules = await db
    ...
    .orderBy(desc(eligibilityRules.effectiveDate), desc(eligibilityRules.id));

  const active = rules.find((r) => r.active) ?? null;
            ...
            <tbody>
              {rules.map((r) => (
                <tr key={r.id} className="align-top hover:bg-slate-50/70">
                  <Td className="font-medium text-slate-900">{r.ruleVersion}</Td>
                  <Td className="whitespace-nowrap text-slate-600">
                    {r.minimumAge}–{r.maximumAge}
                  </Td>
                  <Td className="text-slate-600">{r.minimumWeightKg} kg</Td>
                  <Td className="whitespace-nowrap text-slate-600">
                    {r.defaultDonationIntervalDays} / {r.maleDonationIntervalDays} / {r.femaleDonationIntervalDays}
                  </Td>
                  <Td>
                    <JsonViewer data={r.temporaryDeferralRules} label="JSON" />
                  </Td>
                  <Td className="whitespace-nowrap text-slate-600">
                    {fmtDate(r.effectiveDate, lang)}
                  </Td>
                  ...
                  <Td>
                    {canManage && !r.active ? (
                      <ActionButton
                        ...
                        confirmTitle={
                          lang === "bn"
                            ? `সংস্করণ ${r.ruleVersion} সক্রিয় করবেন?`
                            : `Activate version ${r.ruleVersion}?`
                        }
                        confirmBody={
                          lang === "bn"
